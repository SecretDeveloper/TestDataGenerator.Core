...
'5926 Surrey Place, Douglas County, Nevada, BIBK.'
...
- Single repeating symbols using the following syntax
  ...
  - Produces items like *'Letters VMYXDGHWFFEZSXNFIVUK and Numbers 869515559387'*.
- Repeating patterns containing multiple letters or numbers of random length.
  - `tdg -t '\<<(\L){5}>>'` - Will generate 5 random upper-case characters. e.g. *'QKSUG'*
  - `tdg -t '\<<(\L\L\d){24}>>'`  - Will generate 24 repeating letter-letter-number values e.g. *'YW3YC3XY5DV8JA5NA7FO3SO7SW8IK3IE7YJ9ML7HA6PL6SQ3IF5QN2XF2PF2MO5GV6DK6OB5'*
- Variable length data can be generated also
  - `tdg -t '\<<(\L){10,20}>>'` - Will generate a string containing between 10 and 20 characters of random value e.g. *'XNRDDAAFPA'*
  - `tdg -t 'Letters \<<\L{2,20}>> and Numbers \<<\d{2,12}>>'` produces items like *'Letters TRRMA and Numbers 782'*
- Input can contain several placeholders.
  ...
  - Produces 100 items like *'Hi there Niapa Otp how are you doing?  Your SSN is 720-15-1085.'*
- Generate 100 SSN like values and output to console window.
  - `tdg -t '\<<[1-9]\d\d-\d\d-\d\d\d\d>>' -c 100`
  - Produces 100 items like *'D51-96-4581'*.
- Generate 100 strings with random name like values and output to file.
  ...
  - Produces 100 items like *'Jiba Oooziayha'*.
- `tdg -t '\<<Letters \w{2,20} and Numbers \d{2,12}\n>>'` produces the following output: *'Letters ccNOcEpbAXWORweV and Numbers 1896156625
'*
...
### Other patterns:
- `'\<<This is a \L\L string>>'` will produce something similar to *'This is a ND string'*.
- `'\<<This is a \d{19} string>>'` will produce something similar to *'This is a 4387750070780624500 string'*.
...
- Patterns can be repeated a specific number of times using the syntax `(\L\d){10}` which will generate 10 repeated letter-number pairs e.g. *'R8U1K1U2R9J8G5H0O4R9'*.
- Patterns can be repeated a random number of times using the syntax `(\L\d){10,20}` which will generate between 10 and 20 repeated letter-number pairs e.g. *'V0D2X3I1H3T1Z8H6D4Q9H3D3C5H7Q6Y8B3'*.
