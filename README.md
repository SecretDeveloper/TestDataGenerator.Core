...
'43 Railroad Terrace, Charles County, Utah, vB€T@'
...
- Single repeating symbols using the following syntax
  ...
  - Produces items like *'Letters GBGGTWCHWLXPUEVYXGIQ and Numbers 552237550440'*.
- Repeating patterns containing multiple letters or numbers of random length.
  - `tdg -t '<<(\L){5}>>'` - Will generate 5 random upper-case characters. e.g. *'HCRMN'*
  - `tdg -t '<<(\L\L\d){24}>>'`  - Will generate 24 repeating letter-letter-number values e.g. *'BP9GD6ZY6ZG7YJ0KX3SW9YZ2MF4AV3RN3LV6JJ4YF3SU3VE0IS3SZ6NC6TT9FZ1HH7LN5MB5'*
- Variable length data can be generated also
  - `tdg -t '<<(\L){10,20}>>'` - Will generate a string containing between 10 and 20 characters of random value e.g. *'TQQGEXRBISCYZOSXSAPQ'*
  - `tdg -t 'Letters <<\L{2,20}>> and Numbers <<\d{2,12}>>'` produces items like *'Letters TGFKNQJQOWR and Numbers 72988480882'*
- Input can contain several placeholders.
  ...
  - Produces 100 items like *'Hi there Aeo Qiimairhh how are you doing?  Your SSN is 784-02-3708.'*
- Generate 100 SSN like values and output to console window.
  - `tdg -t '<<[1-9]\d\d-\d\d-\d\d\d\d>>' -c 100`
  - Produces 100 items like *'W75-68-0101'*.
- Generate 100 strings with random name like values and output to file.
  ...
  - Produces 100 items like *'Loao Kezhailzo'*.
- `tdg -t '<<Letters \w{2,20} and Numbers \d{2,12}\n>>'` produces the following output: *'Letters NxftEnfRYFWU and Numbers 381428486
'*
...
For example `\L{5}` will produce 5 upper case letters.  You can also add some randomness to the mix by supplying a range: `\L{min,max}`.  The pattern `\L{1,100}` will produce between 1 and 100 upper case letters. Here's one *'ZMFDYDEPWVGVFUQOUCPRKWMORMBEDEHRCBDXJDE'*
...
`(\l\d){5}` will produce something like *'r8a0c4m6j7'*.
...
### Other patterns:
- `'<<This is a \L\L string>>'` will produce something similar to *'This is a SA string'*.
- `'<<This is a \d{19} string>>'` will produce something similar to *'This is a 3299196440046556167 string'*.
...
- Patterns can be repeated a specific number of times using the syntax `(\L\d){10}` which will generate 10 repeated letter-number pairs e.g. *'F6Y9W3U4E1V6K4E8L1C9'*.
- Patterns can be repeated a random number of times using the syntax `(\L\d){10,20}` which will generate between 10 and 20 repeated letter-number pairs e.g. *'S6S5U5K5W7M7A5I4I0O3O4I6K6A4R1O0R8M0F3'*.
