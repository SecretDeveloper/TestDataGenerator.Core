...
'86 Elmwood Drive, Putnam County, Utah, 80379'
...
- Single repeating symbols using the following syntax
  ...
  - Produces items like *'Letters VNDQHUVVKRVEHYDXKZSF and Numbers 148353815817'*.
- Repeating patterns containing multiple letters or numbers of random length.
  - `tdg -t '<<(\L){5}>>'` - Will generate 5 random upper-case characters. e.g. *'LZSFD'*
  - `tdg -t '<<(\L\L\d){24}>>'`  - Will generate 24 repeating letter-letter-number values e.g. *'ME9MR4FW4BP1ZM0PN8JS8BZ9QG5OB7IF9UY8VS4ZP4CG2BP3CE8CJ5HL1ZW4YR7TS3FE5FU8'*
- Variable length data can be generated also
  - `tdg -t '<<(\L){10,20}>>'` - Will generate a string containing between 10 and 20 characters of random value e.g. *'JCJYSROQELXWFW'*
  - `tdg -t 'Letters <<\L{2,20}>> and Numbers <<\d{2,12}>>'` produces items like *'Letters MMQWWABXR and Numbers 588488'*
- Input can contain several placeholders.
  ...
  - Produces 100 items like *'Hi there Jzi Yitfw how are you doing?  Your SSN is 547-70-5993.'*
- Generate 100 SSN like values and output to console window.
  - `tdg -t '<<[1-9]\d\d-\d\d-\d\d\d\d>>' -c 100`
  - Produces 100 items like *'C26-51-7723'*.
- Generate 100 strings with random name like values and output to file.
  ...
  - Produces 100 items like *'Oiti Xuozaipge'*.
- `tdg -t '<<Letters \w{2,20} and Numbers \d{2,12}\n>>'` produces the following output: *'Letters OzbudKPqxhnIkm and Numbers 245536319681
'*
...
For example `\L{5}` will produce 5 upper case letters.  You can also add some randomness to the mix by supplying a range: `\L{min,max}`.  The pattern `\L{1,100}` will produce between 1 and 100 upper case letters. Here's one *'KIJDAN'*
...
`(\l\d){5}` will produce something like *'x0d3q5o2b6'*.
...
### Other patterns:
- `'<<This is a \L\L string>>'` will produce something similar to *'This is a IG string'*.
- `'<<This is a \d{19} string>>'` will produce something similar to *'This is a 7208107904317257918 string'*.
...
- Patterns can be repeated a specific number of times using the syntax `(\L\d){10}` which will generate 10 repeated letter-number pairs e.g. *'D6R3V1C9Z8L2M6J0W9N2'*.
- Patterns can be repeated a random number of times using the syntax `(\L\d){10,20}` which will generate between 10 and 20 repeated letter-number pairs e.g. *'K0N4S0P3A8I9V1P1Z2S0B7C5D8'*.
