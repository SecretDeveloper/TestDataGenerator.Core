...
'3 Steeplechase Boulevard, Loudoun County, Maine, 45407'
...
- Single repeating symbols using the following syntax
  ...
  - Produces items like *'Letters RQMEATFSEKLHVLVSIEQG and Numbers 691870916767'*.
- Repeating patterns containing multiple letters or numbers of random length.
  - `tdg -t '<<(\L){5}>>'` - Will generate 5 random upper-case characters. e.g. *'PKEQV'*
  - `tdg -t '<<(\L\L\d){24}>>'`  - Will generate 24 repeating letter-letter-number values e.g. *'XG4YI7VG0AX2RD4VV0RM6JZ7BA4UQ6KG1WL9DH3HH5UZ0YA6FY7JM1EN2RW2IB6IQ6AU7MR5'*
- Variable length data can be generated also
  - `tdg -t '<<(\L){10,20}>>'` - Will generate a string containing between 10 and 20 characters of random value e.g. *'KFCPZTDAUPSXBTARX'*
  - `tdg -t 'Letters <<\L{2,20}>> and Numbers <<\d{2,12}>>'` produces items like *'Letters LLCZGYLJKXFXTG and Numbers 6091'*
- Input can contain several placeholders.
  ...
  - Produces 100 items like *'Hi there Ysgi Miiadoep how are you doing?  Your SSN is 911-63-2888.'*
- Generate 100 SSN like values and output to console window.
  - `tdg -t '<<[1-9]\d\d-\d\d-\d\d\d\d>>' -c 100`
  - Produces 100 items like *'L63-60-5945'*.
- Generate 100 strings with random name like values and output to file.
  ...
  - Produces 100 items like *'Sulo Auuouacza'*.
- `tdg -t '<<Letters \w{2,20} and Numbers \d{2,12}\n>>'` produces the following output: *'Letters eGZY and Numbers 6544664711
'*
...
For example `\L{5}` will produce 5 upper case letters.  You can also add some randomness to the mix by supplying a range: `\L{min,max}`.  The pattern `\L{1,100}` will produce between 1 and 100 upper case letters. Here's one *'NVJSKTAECDUUJSTWKNHBCQDTYGJTENFPJZOGPITIPVWRWVDTYTCGXCVXSQUDKHAPYEKJEMHCUZMHTMXCDQUKLYSHEQOZ'*
...
`(\l\d){5}` will produce something like *'o6y3v2s6k7'*.
...
### Other patterns:
- `'<<This is a \L\L string>>'` will produce something similar to *'This is a PM string'*.
- `'<<This is a \d{19} string>>'` will produce something similar to *'This is a 4949651276074619411 string'*.
...
- Patterns can be repeated a specific number of times using the syntax `(\L\d){10}` which will generate 10 repeated letter-number pairs e.g. *'N9I5E0E4M7F7J0I9L4U6'*.
- Patterns can be repeated a random number of times using the syntax `(\L\d){10,20}` which will generate between 10 and 20 repeated letter-number pairs e.g. *'M3G8O1V2D7F2O0O1L3X4C6L5Z6S1R6O6'*.
